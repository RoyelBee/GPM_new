...
def executives_brand_target_sales_chart(name):
    ...
    for j in range(0, np.shape(all_data)[1]):
        ...
        pd.DataFrame(vars()['new_list' + str(j)]).to_excel(writer, sheet_name='Sheet2', index=False, startcol=j, startrow=0)
    writer.save()
    ...
    for value in new_all_data2:
        if value <= 0:
            index=new_all_data2.index(value)
            del new_all_data2[index]
    print(new_all_data2)
    ...
    def zero_label_checker(val, label,val2):
        if val == 0.0:
            lable = ''
        else:
            lable = label + '\n' + str(round(val,2)) + '%'+ '\n'+str(val2)
            ...
            for axis in axes:
                for bar in axis:
                    ...
                    plt.text(bar.get_x() + w / 2, bar.get_y() + h / 2,
                             zero_label_checker(h, brands[i],new_all_data2[i]), ha="center",
